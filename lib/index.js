...
class MongoSocketClient {
    ...
    */
     ...
     */
    replaceOne(collection, queryObject, fieldsAndValues) {
        return new Promise((resolve, reject) => {
            const updateObjects = this.socketClient.emit(server_constants_1.EMMITER.REPLACE_ONE, this.databaseName, collection, queryObject, fieldsAndValues);
     ...
     * @result information from updated objects
     */
    updateOne(collection, queryObject, fieldsAndValues) {
        return new Promise((resolve, reject) => {
            const updateObjects = this.socketClient.emit(server_constants_1.EMMITER.UPDATE_ONE, this.databaseName, collection, queryObject, fieldsAndValues);
            updateObjects.on(server_constants_1.ON.STATUS_SUCCESS, (value) => {
                resolve(value);
            });
            updateObjects.on(server_constants_1.ON.STATUS_FAIL, (reason) => {
     ...
     * @result information from updated objects
     */
    updateMany(collection, queryObject, fieldsAndValues) {
        return new Promise((resolve, reject) => {
            const updateObjects = this.socketClient.emit(server_constants_1.EMMITER.UPDATE_MANY, this.databaseName, collection, queryObject, fieldsAndValues);
            updateObjects.on(server_constants_1.ON.STATUS_SUCCESS, (value) => {
                resolve(value);
            });
            updateObjects.on(server_constants_1.ON.STATUS_FAIL, (reason) => {
    ...
    subscribeCollection(collection) {
        return new rxjs_1.Observable((subscriber) => {
            this.socketClient.emit(server_constants_1.EMMITER.SUBSCRIBE_COLLECTION, this.databaseName, collection)
                .on(server_constants_1.ON.COLLECTION_CHANGED, (doc) => {
                subscriber.next(doc);
            });
        });
    }
